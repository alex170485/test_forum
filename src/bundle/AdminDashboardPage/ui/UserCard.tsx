    ...
    <Widget>
      <Box width='100%' display="flex" flexDirection="column">
        ...
        <Box display='flex' flexDirection='column' justifyContent='space-between'>
          <UserCardRow title='Имя'>
            {user?.firstName}
          </UserCardRow>
          <UserCardRow title='Отчество'>
            {user?.middleName || '-'}
          </UserCardRow>
          <UserCardRow title='Фамилия'>
            {user?.lastName || '-'}
          </UserCardRow>
          <UserCardRow title='Ник нейм'>
            {user?.nickname || '-'}
          </UserCardRow>
          <UserCardRow title='Статус'>
            {currentStatus}
          </UserCardRow>
          <UserCardRow title='Роль'>
            {currentRole}
          </UserCardRow>
          <UserCardRow title='Уровень'>
            {user?.level || '-'}
          </UserCardRow>
        </Box>
      </Box>
